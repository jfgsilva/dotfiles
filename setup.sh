...
if [ "$USER_CHOICE" = "full" ] || [ "$USER_CHOICE" = "config" ]; then
  ...
  ln -sf "$SCRIPT_DIR/nvim-plugins/parrot.lua" "$XDG_CONFIG_HOME"/nvim/lua/custom/plugins/
  ln -sf "$SCRIPT_DIR/nvim-plugins/diffview.lua" "$XDG_CONFIG_HOME"/nvim/lua/custom/plugins/
fi
